...
- **麦麦情绪联动**（v1.31.0，可选）：与 astrbot_plugin_maisoul 双向数值耦合——评审模型跟随麦麦 replyer 实际使用的模型（打分的就是说话的）；连续同向情绪放大/缩小评审增益（同向最高 ×2、异向 ÷2）；好感等级跃迁反向注入情绪事件（升级→安心/开心，降级→委屈/悲伤）——评审在后台进行，好感数值影响下一条消息的注入，跃迁情绪在随后几分钟（半衰期约 7 分钟）内影响语气。只交换数值不注入提示词，麦麦不在场或开关关闭时自动空转
...
| judge.follow_maisoul_replyer | true | 检测到麦麦插件时，评估模型跟随其 replyer 实际使用的模型（人格口径一致且必然可用）——该跟随**优先于** narrative_reason 的强制会话模型与 provider_id 配置；麦麦未装/解析失败才静默回落到：narrative_reason 强制会话模型 > provider_id > 会话主模型 |
...
| judge.roster | "" | 群成员花名册（每行一条，如 `阿狸=123456789（群友外号示例）`），注入评审提示词，避免把熟人玩笑误判为敌意 |
| impression.interval | 8 | 每累计 N 次有效评估自动刷新一次该成员印象（后台静默）；手动：`/印象刷新 QQ号` 或面板按钮 |
| economy.* | 见 schema | 防通胀经济学（仅评审路径）：噪声地板 0.5 / 负面权重 1.5 / 同日重复衰减 0.25 / 阶段乘数（挚友 0.35、挚爱 0.2）/ 信任修复期（重大得罪后 48h 内正分半效——得罪了要哄好几天） |
...
| levels.* | 见 schema | 七级阈值（含「厌恶」负值区间）与态度指引文本；每级另有 master_guidance（主人专属指引，负好感＝闹别扭，正值＝亲疏程度）与 disclosure（自我表露分寸，关系越深袒露层越深） |
| inject.enabled | true | 提示词注入开关；template 可自定义 |
| inject.memory_* | 3 / 7 / 1.0 / 3.0 | 近期印象：条数 / 窗口天数 / 显著性阈值 / 显著事件记忆窗口倍数——大事（被重骂/被真诚打动）记得久，小寒暄照常淡忘；已撤销的变动不注入 |
