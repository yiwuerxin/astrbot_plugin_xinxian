...
**规则事件与 LLM 情绪评估双驱动** · 陌生到挚爱六级好感阶段 · 每群独立记录 · 对话时自动注入态度指引 · 指令 / LLM 工具 / 跨插件 API 三通道。

## 功能特性

- **双引擎增减**：明确事件规则（被夸/被骂/@/回复/每日首聊）+ LLM 情绪评估（判断对小千的态度微调，可关）
- **六级好感阶段**：陌生 → 认识 → 友好 → 亲密 → 挚友 → 挚爱，每级附带态度指引
- **每群独立**：同一 QQ 在不同群的好感度分开记录（主键 = 群号 + QQ 号）
- **主人身份**：按 QQ 号认定（昵称不可信），主人有专属身份标识，但数值照常涨跌
- **提示词注入**：每次对话自动把对方的好感度档案注入 system_prompt，小千自然地对亲密的人更软、对陌生的人有距离
- **防刷机制**：单事件冷却 + 每日双向限幅（净增/净降各限）+ 数值封顶
...
└── tests/test_core.py       # 核心层单元测试（18 个用例）
```
...
| max_favor / default_favor | 100 / 0 | 好感度上限 / 初始值 |
| daily_cap_up / daily_cap_down | 15 / 15 | 每日净增/净降上限 |
| rules.* | 见 schema | 五类规则事件的开关、分值、冷却、关键词 |
| judge.enabled / provider_id | true / "" | LLM 评估开关与模型（建议选便宜小模型，留空跟随会话） |
| judge.only_when_at_or_reply | true | 仅@/回复时评估，省成本 |
| levels.* | 见 schema | 六级阈值与态度指引文本 |
